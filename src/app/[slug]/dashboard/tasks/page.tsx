...
export default function TasksNotesPage() {
    ...
    const fetchTasks = async () => {
        ...
        const { data, error } = await supabase.from('tasks').select('*').eq('store_id', activeStore.id).order('created_at', { ascending: false });
        if (error) {
            console.error('Error info:', error);
            // Silent fail for now, maybe table does not exist yet.
        } else if (data) setTasks(data);
    };

    const fetchNotes = async () => {
        if (!activeStore) return;
        const { data, error } = await supabase.from('notes').select('*').eq('store_id', activeStore.id).order('is_pinned', { ascending: false }).order('created_at', { ascending: false });
        if (!error && data) setNotes(data);
    };

    const fetchEmployees = async () => {
        if (!activeStore) return;
        const { data, error } = await supabase.from('employees').select('id, name').eq('store_id', activeStore.id);
    ...
    const handleSaveNote = async (e: React.FormEvent) => {
        ...
        try {
            if (editingNote) {
                const { error } = await supabase.from('notes').update(noteForm).eq('id', editingNote.id);
                if (error) throw error;
