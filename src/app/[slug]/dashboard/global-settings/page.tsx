...
import { Palette, Globe, Plus, Store, Archive as ArchiveIcon, Edit2, RotateCcw, Trash2, RefreshCw, Save, ShieldAlert, X, ArrowUp, ArrowDown } from 'lucide-react';
import ConfirmDialog from '@/components/ui/ConfirmDialog';
import { supabase } from '@/lib/supabase';

export default function GlobalSettingsPage() {
    const { user, globalSettings, updateGlobalSettings, stores, createStore, updateStoreStatus, requestStoreDeletionOTC, verifyStoreDeletionOTC, deleteStore, updateStoreOrder } = useAuth();
    ...
    const handleMoveUp = async (index: number) => {
        ...
        // Prepare updates
        const updates = newStores.map((s, i) => ({ id: s.id, sort_order: i + 1 }));
        await updateStoreOrder(updates);
    };
                        ...
                        <div key={store.id} className={`bg-white dark:bg-slate-900 rounded-xl border p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 transition-all ${store.status === 'archived' ? 'border-amber-200 bg-amber-50/50 dark:border-amber-900/30' : 'border-slate-200 dark:border-slate-800'}`}>
                            ...
                            <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                                <div className="flex items-center gap-1 mr-2 border-r border-slate-200 dark:border-slate-800 pr-3">
                                    <button
                                        onClick={() => handleMoveUp(stores.indexOf(store))}
                                        disabled={stores.indexOf(store) === 0}
                                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                        title="Move Up"
                                    >
                                        <ArrowUp className="h-4 w-4" />
                                    </button>
                                    <button
                                        onClick={() => handleMoveDown(stores.indexOf(store))}
