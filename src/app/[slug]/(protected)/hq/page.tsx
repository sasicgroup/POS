...
export default function HQDashboardPage() {
    ...
    useEffect(() => {
        const loadHQData = async () => {
            ...
            try {
                // 1. Fetch Fresh Store List (Scoped to Business)
                const businessId = typeof localStorage !== 'undefined' ? localStorage.getItem('sms_business_id') : null;
                let query = supabase.from('stores').select('*').neq('status', 'deleted');
                if (businessId) query = query.eq('business_id', businessId);
                
                const { data: freshStores, error: storesError } = await query.order('sort_order', { ascending: true });
