...
export default function SalesPage() {
    const { activeStore, user, updateStoreSettings, hasPermission, businessId } = useAuth();
    const {
        products,
        isLoading,
        processSale,
    ...
    const handlePrintReceipt = (transactionId: string) => {
        if (!activeStore) return; // Guard: activeStore required for printing
    ...
    const handleCheckout = async () => {
        if (!activeStore) {
            showToast('error', 'No active store selected');
            return;
        }
