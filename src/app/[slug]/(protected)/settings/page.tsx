...
export default function SettingsPage() {
    const { activeStore, hasPermission, user, updateStoreSettings, teamMembers, addTeamMember, updateTeamMember, removeTeamMember, globalSettings, updateGlobalSettings, updateUser, businessId } = useAuth();
    ...
    const fetchBarcodeLibrary = async () => {
        if (!activeStore?.id) return;
        let query = supabase.from('generated_barcodes').select('*').eq('store_id', activeStore.id);
    ...
    const handleUpdatePin = async () => {
        ...
        if (user?.id) {
            await supabase.from('employees').update({ pin: newPin }).eq('id', user.id).eq('business_id', businessId);
