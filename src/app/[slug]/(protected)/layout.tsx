...
function DashboardContent({ children }: { children: React.ReactNode }) {
    ...
    const navigation = [
        ...
        { name: 'Inventory', href: `/${slug}/inventory`, icon: Package, permission: 'view_inventory' },
        { name: 'Sales / POS', href: `/${slug}/sales`, icon: ShoppingBag, permission: 'process_sales' },
        { name: 'Invoices', href: `/${slug}/invoices`, icon: FileText, permission: 'manage_invoices' },
        { name: 'Tasks & Notes', href: `/${slug}/tasks`, icon: ListTodo, permission: 'access_tasks' },
        { name: 'Sales History', href: `/${slug}/sales/history`, icon: CalendarClock, permission: 'view_sales_history' },
        { name: 'Installments', href: `/${slug}/installments`, icon: CreditCard, permission: 'manage_installments' },
        { name: 'AI Insights', href: `/${slug}/ai-insights`, icon: Sparkles, permission: 'access_ai_insights' },
        { name: 'Loyalty Program', href: `/${slug}/loyalty`, icon: Award, permission: 'access_loyalty' },
        { name: 'Customers', href: `/${slug}/customers`, icon: Users, permission: 'view_customers' },
        { name: 'Employees', href: `/${slug}/employees`, icon: Users, permission: 'view_employees' },
        { name: 'Income / Expenses', href: `/${slug}/income-expenses`, icon: Receipt, permission: 'view_financials' },
        { name: 'Reports', href: `/${slug}/reports`, icon: TrendingUp, permission: 'access_reports' },
        { name: 'Activity Logs', href: `/${slug}/logs`, icon: Activity, permission: 'access_logs' },
        { name: 'Roles & Permissions', href: `/${slug}/roles`, icon: ShieldCheck, permission: 'view_roles' },
        { name: 'SMS / WhatsApp', href: `/${slug}/sms`, icon: MessageSquare, permission: 'send_messages' },
        { name: 'HQ Dashboard', href: `/${slug}/hq`, icon: Globe, permission: 'owner_only' },
        { name: 'Global Settings', href: `/${slug}/global-settings`, icon: Globe, permission: 'owner_only' },
        { name: 'Settings', href: `/${slug}/settings`, icon: Settings, permission: 'access_settings' },
    ];
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
            ...
            <aside className={`fixed inset-y-0 left-0 z-30 w-64 transform border-r border-slate-200 bg-white/80 backdrop-blur-md transition-transform duration-300 ease-in-out dark:border-slate-800 dark:bg-slate-950/80 lg:static lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex h-full flex-col">
                    ...
                    <div className="flex h-16 items-center border-b border-slate-200 px-6 dark:border-slate-800 gap-3">
                        ...
                        >
                            {globalSettings?.appName || 'Business Portal'}
                        </span>
                    </div>
                        ...
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div className="overflow-hidden">
                                <p className="truncate text-sm font-medium text-slate-900 dark:text-slate-100">
                                    {user.role === 'owner' && globalSettings.ownerName ? globalSettings.ownerName : user.name}
                                </p>
                                <p className="truncate text-xs text-slate-500 dark:text-slate-400 capitalize">
                                    {user.role === 'owner' ? 'Business Owner' : (user.role || 'Staff').replace('_', ' ')}
                                </p>
                            </div>
                        </div>
                        <button
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 flex h-16 items-center justify-around border-t border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 px-1 dark:border-slate-800 dark:bg-slate-950/95 lg:hidden shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] pb-safe">
                {[
                    ...
                    { name: 'Inventory', href: `/${slug}/inventory`, icon: Package, permission: 'view_inventory' },
                    { name: 'Sales', href: `/${slug}/sales`, icon: ShoppingBag, permission: 'access_pos' },
                    { name: 'Customers', href: `/${slug}/customers`, icon: Users, permission: 'view_customers' },
                    { name: 'Settings', href: `/${slug}/settings`, icon: Settings, permission: 'access_settings' },
                ].filter(item => {
                    if (!user) return false;
